...
class ChatTests {
  ...
  async testSendMessage() {
    await this.runTest('Send Message - Offline Delivery', async () => {
      ...
      const messageData = {
        recipientId: '507f1f77bcf86cd799439011', // Mock recipient ID
        roomId: '507f1f77bcf86cd799439012', // Mock room ID
  ...
  async testMarkMessageDelivered() {
    await this.runTest('Mark Message Delivered', async () => {
      const user = this.testUsers[0];
      const messageId = '507f1f77bcf86cd799439013'; // Mock message ID
      ...
      // Expect 404 since message doesn't exist, but validates endpoint
      this.helpers.assertError(response, 404, 'MESSAGE_NOT_FOUND');
    });
  }

  async testCleanupMessage() {
    await this.runTest('Cleanup Delivered Message', async () => {
      const user = this.testUsers[0];
      const messageId = '507f1f77bcf86cd799439013'; // Mock message ID
